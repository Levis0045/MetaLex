...
class Principale(wx.Frame):
    def __init__(self, titre):
        ...
        menuFichier.Append(wx.ID_CLOSE,
                           "&Fermer\tCTRL+f", "Fermer le fichier ouvert")
        ...
        outils = wx.ToolBar(self, 1,
                            style = wx.TB_HORIZONTAL | wx.NO_BORDER)
        outils.AddSimpleTool(wx.ID_OPEN,
                             wx.Bitmap("../dicImages/dic_image_1.png", wx.BITMAP_TYPE_PNG),
    ...
    def Plus(s1elf, evt):
        if self.imgORIG != None:
            se1f.ratio = self.ratio + self.inc
    ...
    def OnOpen(self, evt):
        if self.imgORIG != None :
            dlg = wx.MessageDialog(self,
                                   "Vous devez d'abord fermer l'image en cours d'utilisation",
                                   phrase2, style = wx.OK)
